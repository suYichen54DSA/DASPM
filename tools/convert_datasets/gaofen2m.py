...
def convert_to_train_id(file):
    ...
    for k, v in id_to_trainid.items():
        k_mask = label == k
        label_copy[k_mask] = v    #基于像元建立映射
        n = int(np.sum(k_mask))
        if n > 0:
            sample_class_stats[v] = n
    
    new_file = file
    new_file = "data/gaofen/label/" + new_file.split("/")[-1]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert gaofen annotations to TrainIds')
    parser.add_argument('gaofen2m_path', default = 'data/gaofen/',nargs='?',help='gaofen data path')

    parser.add_argument('--gaofen-dir', default='label', type=str)
...
def main():
    args = parse_args()
    gaofen_path = args.gaofen2m_path
    out_dir = args.out_dir if args.out_dir else gaofen_path
    mmcv.mkdir_or_exist(out_dir)

    gaofen_dir = osp.join(gaofen_path, args.gaofen_dir)

    poly_files = []
    for poly in mmcv.scandir(
            gaofen_dir, suffix=tuple(f'{i}.tif' for i in range(10)),
            recursive=True):
        poly_file = osp.join(gaofen_dir, poly)
